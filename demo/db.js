    ...
    function getProp(o) {
        for(var prop in o) {
            if(typeof(o[prop]) === 'object') {
            	...
            	if (prop != "lat" && prop != "lng" && prop != "timestamp"){
                _r.push(o[prop]);
                }
            }
        }
  ...
  for (let i = 0; i < notes.length; i++) {
    ...
    if (_JNum > 1){
        for (let i = 0; i<(_JNum-1); i++){
            ...
            let NewString = $('<tr class="card jr-string added"></tr>').insertAfter(A);
            let NewStringLeftCell = $('<td></td>').appendTo(NewString);
            let _Text = $('<textarea class="jr" rows="1" placeholder="ООО «А-Стиль»"></textarea>').appendTo(NewStringLeftCell);
    ...
    if (_CNum > 2){
        for (let i = 0; i<(_CNum-1); i+=2){
            let A = $(_cardDiv).children().find("input.contact").eq(i);
            let T = A.parents("tr[class$='string']").children().eq(0);
            let NewString = $('<tr class="card contact-string added"></tr>').insertAfter(A);
            let NewStringLeftCell = $('<td></td>').appendTo(NewString);
            let C = $('<input class="contact contact-name" placeholder="Анна Андреева"/><BR><input class="contact contact-number" placeholder="+7(012)345-67-89, доб. 12345"/><BR>').appendTo(NewStringLeftCell);
            $('<td class="plus-minus"><img src="../imgs/minus.svg"/></td>').appendTo(NewString);
            T.prop("rowspan", T.prop("rowspan")-1+2);
        }
    }
    //console.log('JR = '+_JNum + ', PLACE = '+_PNum+', CONTACT = '+_CNum);
    ...
    $(_cardDiv).children().find("input, textarea").each(function( index ) {
        $(this).replaceWith(function(){
            ...
            if (_cardArray[0].match(/line.+/)){
                ...
                _cardArray[0] = _cardArray[0].match(/\x20.+/);
            } else {
                _PClass = '';
            }
...
const updateNote = (event) => {
  ...
  //console.dir(getAnObject($(event.target).parents(".leaflet-popup-edit-screen").find("table.card").eq(0)));
  req.onsuccess = (event) => {  
